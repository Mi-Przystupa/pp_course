(ns assign4.constants)

(def message "Successfuly Parsed: ")
(def prog1 "(let [x (sample (normal 0))] x)")
(def prog2 "(let [data (vector 1 2 3)
                a (vector 2)]
            (vector (first (rest (rest data))) a))")
(def prog3 "(let [data (vector 1 2 (sample (normal 1 1)))
      a (conj [] (sample (normal 0 2)))
      b (conj a (sample (normal 0 3)))]
  (observe (normal (second b) 4) (first (rest data))) b)")
(def eval1 "(let [mu (sample (normal 1 (sqrt 5)))
           sigma (sqrt 2)
           lik (normal mu sigma)]
       (observe lik 8)
       (observe lik 9)
...
(def eval2 "(defn observe-data [_ data slope bias]
        (let [xn (first data)
              yn (second data)
              zn (+ (* slope xn) bias)]
...
(def eval3 "(defn hmm-step [t states data trans-dists likes]
...
(def eval4 "(let [weight-prior (normal 0 1)
      W_0 (foreach 10 []
